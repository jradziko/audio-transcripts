...
def main():
    ...
    # Get Hugging Face token from file
    try:
    ...
    if not verify_hf_token(hf_token):
        print("Invalid Hugging Face token. Please ensure your token is correct and try again.")
        sys.exit(1)
    print("Hugging Face token verified successfully.\n")
    ...
    # Step 5: Transcribe with Whisper using GPU (CUDA)
    print("Starting transcription with Whisper...")
    try:
        model = whisper.load_model("medium", device="cuda")  # Load Whisper with GPU
